...
class TextCommands:
    ...
    def add_text(self, args: list) -> str:
        if len(args) < 2:
            return "Usage: add_text <container|leaf> <type>"
        ...
        if element_type == "container" and specific_type == "heading":
            ...
            level = int(input("Enter heading level (1-6): ") or "1")
            element_id = self.text_factory.add_heading(content, level)
            return f"Element ID: {element_id}"

        elif element_type == "leaf" and specific_type == "paragraph":
            content = input("Enter paragraph content: ")
            element_id = self.text_factory.add_paragraph(content)
            return f"Element ID: {element_id}"
    ...
    def save(self, args: list) -> str:
        ...
        self.text_factory.save_document(args[0])
        return ""

    def load(self, args: list) -> str:
        if not args:
            return "Usage: load <name>"
        self.text_factory.load_document(args[0])
        return ""